...
agent_name = "okta_sample_agentcore_with_gateway"
...
# Optionally attach Lambda interceptor to the Gateway (so Gateway adds Authorization from X-ID-Token).
# Requires: ATTACH_INTERCEPTOR=true and GATEWAY_* / INTERCEPTOR_LAMBDA_ARN in .env.
# Deploy the Lambda first (see gateway_interceptor/README.md); then set INTERCEPTOR_LAMBDA_ARN.
def attach_interceptor_to_gateway():
    ...
    lambda_arn = os.getenv("INTERCEPTOR_LAMBDA_ARN", "").strip()
    ...
    if not all([gateway_id, gateway_name, gateway_role_arn, lambda_arn, discovery_url, allowed_audience]):
        print(
            ...
            "INTERCEPTOR_LAMBDA_ARN, DISCOVERY_URL, OKTA_AUDIENCE is missing. Skipping gateway update."
        )
        return
    try:
        ...
        print("Gateway interceptor attached successfully:", gateway_id)
    except Exception as e:
        print("Warning: failed to attach interceptor to gateway:", e)
        traceback.print_exc()


attach_interceptor_to_gateway()
